...
import { findBestStateForCitySlug, cityExistsInState, normalizeCitySlug } from '@/utils/cityIndex';

// Comprehensive mapping of legacy service patterns to current service slugs
const legacyServiceMappings: Record<string, string> = {
...
const metroAreaMappings: Record<string, { state: string; city: string }> = {
...
const countyMappings: Record<string, { state: string; city: string }> = {
...
interface LegacyRedirectResult {
  redirectTo: string;
  permanent: boolean;
  status?: number;
  noIndex?: boolean;
...
export const parseLegacyUrl = (pathname: string): LegacyRedirectResult | null => {
  ...
  // Pattern 1: /service-location/ (e.g., /forensic-economist-california/)
  if (segments.length === 1) {
    return handleServiceLocationPattern(segments[0]);
  }

  // Pattern 2: /service-type/location/ (e.g., /business-valuation/california/)
  if (segments.length === 2) {
    return handleServiceDirectoryPattern(segments[0], segments[1]);
  }

  // Pattern 3: /service-type/state/city/ (e.g., /forensic-economist/california/los-angeles/)
  if (segments.length === 3) {
    return handleServiceStateCity(segments[0], segments[1], segments[2]);
  }

  // Pattern 4: /locations/cities/[city]-[state]-[service].html
  if (segments.length >= 3 && segments[0] === 'locations' && segments[1] === 'cities') {
    return handleLocationCitiesPattern(segments[2]);
  }

  // Pattern 5: /locations/states/[state]/
  if (segments.length >= 3 && segments[0] === 'locations' && segments[1] === 'states') {
    return handleLocationStatesPattern(segments[2]);
  }
  
  // Pattern 5b: /locations/[state]/ (shorthand)
  if (segments.length === 2 && segments[0] === 'locations') {
    return handleLocationStatesPattern(segments[1]);
  }

  // Pattern 6: /practice-areas/service/ or /practice-areas/service/location/
...
function handleServiceLocationPattern(segment: string): LegacyRedirectResult | null {
  ...
  for (const delimiter of delimiters) {
    ...
    if (parts.length >= 2) {
      ...
      // This handles metro areas and counties at the beginning
      ...
      // Try multiple combinations for complex city-state patterns
      for (let i = 1; i < parts.length; i++) {
        ...
        if (legacyServiceMappings[potentialService]) {
          ...
          // Infer state from city-only location using our city index
          const normalizedCity = normalizeCitySlug(potentialLocation);
          const cityMatch = findBestStateForCitySlug(normalizedCity);
          if (cityMatch) {
            if (cityExistsInState(cityMatch.stateSlug, normalizedCity)) {
              return {
                redirectTo: `/services/${currentService}/${cityMatch.stateSlug}/${normalizedCity}`,
                permanent: true
              };
            }
            // City not in dataset for that state – fall back to the state overview page
            return {
              redirectTo: `/${cityMatch.stateSlug}`,
              permanent: true
            };
          }
...
function handleServiceDirectoryPattern(service: string, location: string): LegacyRedirectResult | null {
  ...
  // Infer state from city-only location using our city index
  const normalizedCity = normalizeCitySlug(location);
  const cityMatch = findBestStateForCitySlug(normalizedCity);
  if (cityMatch) {
    if (cityExistsInState(cityMatch.stateSlug, normalizedCity)) {
      return {
...
function handleServiceStateCity(service: string, state: string, city: string): LegacyRedirectResult | null {
  const currentService = legacyServiceMappings[service];
  if (!currentService) return null;
...
function handlePracticeAreasPattern(segments: string[]): LegacyRedirectResult | null {
...
function handleLocationCitiesPattern(segment: string): LegacyRedirectResult | null {
...
function handleLocationStatesPattern(state: string): LegacyRedirectResult | null {
  // Handle state abbreviations
  if (stateAbbreviations[state]) {
    return {
...
function handleComplexCityStatePattern(locationStr: string, currentService: string): LegacyRedirectResult | null {
...
// Examples: philadelphia-metro-economist, cook-county-economist
function handleLocationServicePattern(parts: string[], delimiter: string): LegacyRedirectResult | null {
...
const LegacyUrlHandler = () => {
  ...
  const result = parseLegacyUrl(location.pathname);
  
  if (result) {
    // Handle vendor/bundle paths with 404 and noindex
    if (result.status === 404 && result.noIndex) {
      // Add noindex meta tag for vendor/bundle paths
      document.head.insertAdjacentHTML('beforeend', 
        '<meta name="robots" content="noindex, nofollow" />'
      );
      // Return 404 component or redirect to 404
      return <Navigate to="/404" replace />;
    }
    
    return <Navigate to={result.redirectTo} replace={result.permanent} />;
  }
  
  // If no mapping found, redirect to locations page
  return <Navigate to="/locations" replace />;
};

export default LegacyUrlHandler;
